...
export default class DashLeaflet extends Component {
    constructor(props) {
        ...
        if (this.props.style['height'] === '100%') {
            console.warn(
                'Attention: Map might not display, if height of element and parent are 100%.'
            );
        }
    }
    ...
    static loadBaseLayers(baselayers) {
        ...
        // LayersControl element.
            ...
            return baselayers.map((baselayers, index) => (
                <ReactLeaflet.LayersControl.BaseLayer
                    key={'baselayer_' + baselayers.name}
                    checked={index === 0}
    ...
    static getOptions(options, type) {
        ...
        let fillOpacity =
            options.fillOpacity != null ? options.fillOpacity : 0.2;
                    ...
                    <ReactLeaflet.Popup>
                        <div
                            dangerouslySetInnerHTML={{
                                __html: Array.isArray(layer.popup)
                                    ? layer.popup[index_j]
                                    : layer.popup,
                            }}
                        />
        ...
        return circleMarkers.map((layer, index_i) => {
            ...
            let layerList = layer.geom.map((position, index_j) => (
                <ReactLeaflet.CircleMarker
                    key={'circleMarker' + index_i + '_' + index_j}
                    center={position}
                    radius={
                        Array.isArray(options.radius)
                            ? options.radius[index_j]
                            : options.radius
                    }
    ...
    render() {
        ...
        if (
            (!this.props.baselayer ||
                !Array.isArray(this.props.baselayer) ||
                this.props.baselayer.length === 1) &&
            this.props.lines.length === 0 &&
            this.props.markers.length === 0 &&
            this.props.circleMarkers.length === 0
        ) {
            layers = DashLeaflet.loadBaseLayers(this.props.baselayer);
        } else {
            layers = (
...
DashLeaflet.propTypes = {
    ...
    markers: PropTypes.arrayOf(
        PropTypes.shape({
            title: PropTypes.string,
            geom: PropTypes.arrayOf(PropTypes.arrayOf(PropTypes.number))
                .isRequired,
            popup: PropTypes.oneOfType([
                PropTypes.arrayOf(PropTypes.string),
                PropTypes.string,
     ...
     */
    circleMarkers: PropTypes.arrayOf(
        PropTypes.shape({
            title: PropTypes.string,
            geom: PropTypes.arrayOf(PropTypes.arrayOf(PropTypes.number))
                .isRequired,
